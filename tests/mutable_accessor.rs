...
fn mutables() {
    ...
    assert_eq!(foo.lazish_mut(), "got from the builder", "lazish mutable accessor returns built value");
    ...
    *foo.std_mut() = Some("manually set".to_string());
    assert_eq!(*foo.std(), Some("manually set".to_string()), "manual assignment into mutable accessor");
